    ...
    for epoch in range(epochs):
        ...
        if loss_ins.item() < best_loss + min_delta:
            if loss_ins.item() <= best_loss:
                ...
                best_model_state = copy.deepcopy(net.state_dict())  # Save the best model state
            else:
                continue
        else:
            no_improvement_count += 1
    ...
    for epoch in range(1, epochs + 1):
        ...
        with torch.no_grad():
            for batch in valid_loader:
                batch_input_valid, batch_bond_ret_valid, batch_factor_valid = batch  # Unpack the batch
