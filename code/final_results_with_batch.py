...
print(f"Table 1 run time: {round((time.time() - start) / 60, 2)} min \n")
...
print(f"Figure A2 run time: {round((time.time() - start) / 60, 2)} min \n")
...
for count1,df_ in enumerate([dict_bm['ew']]+
    ...
    for y in [df_[T_INS:T_ALL]]:
        ...
        newey_west_L = np.int(4*(len(y)/100)**(2/9))
...
print(f"Table 3 run time: {round((time.time() - start) / 60, 2)} min \n")
...
print(f"Figure 2 run time: {round((time.time() - start) / 60, 2)} min \n")
...
print(f"Figure 3 run time: {round((time.time() - start) / 60, 2)} min \n")
...
for x,count1,model_name in [(df_ff5,5,'FF5'), (df_ipca,5,'IPCA5'), (df_rppca,5,'RPPCA5')]:
    ...
    for y in [df_sel_ft]:
        ...
        newey_west_L = np.int(4*(len(y2)/100)**(2/9))
        ...
        newey_west_L = np.int(4*(len(ret_2)/100)**(2/9))
...
print(f"Table 4 run time: {round((time.time() - start) / 60, 2)} min \n")
...
for i,row in enumerate(df_char_sum.index):
    newey_west_L = np.int(4*(T_ALL/100)**(2/9))
...
for row in df_char_sum.index:
    if row in df_rename['before'].values:
        df_char_sum.loc[row,'newchar'] = df_rename[df_rename['before']==row]['after'].values
    else:
...
# Filter out Top important variables for RPPCA
df_char_top = df_char.iloc[:50].reset_index()

df_char_top = df_char_top.rename(columns={'index':'rawchar'})
df_char_top.to_csv(f'../data/data_in_all_{update_date}/top_important_variables_{panel_num}.csv', index=False, encoding='utf_8_sig')

print(f"Figure 4 run time: {round((time.time() - start) / 60, 2)} min \n")



#################################################################
##### Complete Table 2: Deep Corporate Bond Factors Panel A #####
#################################################################

start = time.time()

ew_dir = '../robust_check/linear_ranking/results_{}/ew_with_batch/{}'.format(char_type, bm_type)
df_best = pd.read_csv(f'{ew_dir}/best_summary_seed{seed}.csv')
...
for port_type in ['vw','ew']:
    dict_df[port_type] = {}
    output_dir = '../robust_check/linear_ranking/results_{}/{}_with_batch/{}'.format(char_type, port_type, bm_type)
...
lam = 1e-5
iter_index = ['MKT$_{EW}$','R$_{l}^1$','R$_{l}^2$','R$_{l}^3$',
              'MKT$_{VW}$','R$_{d}^1$','R$_{d}^2$','R$_{d}^3$']

df_stat = pd.DataFrame(index =iter_index,columns = ['Mean','Min','Max','tstat','SR','Max DD'])
df_pval = df_stat.copy()

count1 = 0
count2 = 0
for df_ in ([dict_bm['ew']]+[dict_df['ew'][x][:,-1].reshape(-1,1) for x in np.array(list_uncond_best_capm)]+
            [dict_bm['vw']]+[dict_df['vw'][x][:,-1].reshape(-1,1) for x in np.array(list_uncond_best_capm)]):
    count2 = 0
    
    # df_ = df_/(df_[:T_INS].std()*np.sqrt(12)) * 0.1
    var_ = iter_index[count1]
    for y in [df_[T_INS:T_ALL]]:
        Mean = np.mean(y*100)
        Std = np.std(y*100)
        ...
        df_stat.loc[var_,'Mean'] = np.mean(y*100)
        df_stat.loc[var_,'Min'] = np.min(y*100)
        df_stat.loc[var_,'Max'] = np.max(y*100)
        df_stat.loc[var_,'tstat'] = tstats
        df_stat.loc[var_,'Std'] = Std
        df_stat.loc[var_,'SR'] = np.round(Mean/Std*np.sqrt(12),5)
        df_stat.loc[var_,'Max DD'] = np.round(max_dd,5)
        
    count1 =count1+1

df_table2_A = df_stat.applymap(lambda x: ('%.2f' % x))
df_table2_A = '$' +df_table2_A +'$'
df_table2_A['Mean'] = df_table2_A['Mean'].astype(str)+'('+ df_table2_A['tstat'].astype(str) +')'
df_table2_A = df_table2_A[['Mean','SR','Min','Max','Max DD']]
df_table2_A_ew = df_table2_A.loc[['R$_{l}^1$','R$_{l}^2$','R$_{l}^3$']]
df_table2_A_ew.index = ['R$_{l}^1$','R$_{l}^2$','R$_{l}^3$']
df_table2_A_ew[''] = ''
df_table2_A_vw = df_table2_A.loc[['R$_{d}^1$','R$_{d}^2$','R$_{d}^3$']]
df_table2_A_vw.index = df_table2_A_ew.index
df_table2_A = pd.concat([df_table2_A_ew, df_table2_A_vw], axis=1)
df_table2_A.loc[len(df_table2_A), :] = ['']*len(df_table2_A.columns)

df_table2 = pd.concat([df_table2_A, df_table2_B])
df_table2.to_csv(f'../{final_output}/Table_2.csv', index=True, encoding='utf_8_sig')

print(f"Table 2 run time: {round((time.time() - start) / 60, 2)} min \n")
...
print(f"Table 5 run time: {round((time.time() - start) / 60, 2)} min \n")



########################################################
##### Table 6: ML Portfolios and Linear Activation #####
########################################################

start = time.time()

# Load in Panel B and summarize
table_6_Panel_B = pd.read_csv('../robust_check/ML_long_short_linear_activation/Table_6_Panel_B_linear_activation.csv', index_col=0)[:max_layers]

new_cols = [f'L={i}' for i in range(1, max_layers+1)]
table_6_Panel_B_new = pd.DataFrame([table_6_Panel_B['D=1'].tolist()], columns=new_cols)
table_6_Panel_B_new.to_csv('../output/Table_6_panelB.csv', index=False, encoding='utf_8_sig')


table_6_Panel_A = pd.read_csv('../robust_check/ML_long_short_linear_activation/Table_6_Panel_A_ML_Portfolios_ALL_OBS_max_SR.csv')
table_6_Panel_A.rename(columns={'Unnamed: 0': 'Model'}, inplace=True)
table_6_Panel_A = table_6_Panel_A.set_index('Model').T
table_6_Panel_A.rename(columns={'SR_OOS':'Max_SR'}, inplace=True)

table_6_Panel_A_1 = pd.read_csv('../robust_check/ML_long_short_linear_activation/Table_6_Panel_A_ML_Portfolios_ALL_OBS_min_MSE.csv')
table_6_Panel_A_1.rename(columns={'Unnamed: 0': 'Model'}, inplace=True)
table_6_Panel_A_1 = table_6_Panel_A_1.set_index('Model').T
table_6_Panel_A_1.rename(columns={'SR_OOS':'Min_MSE'}, inplace=True)

table_6_Panel_A_2 = pd.read_csv('../robust_check/ML_long_short_linear_activation/Table_6_Panel_A_ML_LS_Portfolios.csv')
table_6_Panel_A_2.rename(columns={'Unnamed: 0': 'Model'}, inplace=True)
table_6_Panel_A_2 = table_6_Panel_A_2.set_index('Model').T
table_6_Panel_A_2.rename(columns={'SR_OOS_10':'10%', 'SR_OOS_20':'20%', 'SR_OOS_30':'30%'}, inplace=True)

table_6_Panel_A_final = pd.concat([table_6_Panel_A[['Max_SR']], table_6_Panel_A_1[['Min_MSE']], table_6_Panel_A_2[['10%','20%','30%']]], axis=1)
table_6_Panel_A_final.to_csv('../output/Table_6_panelA.csv', index=True, encoding='utf_8_sig')

print(f"Table 6 run time: {round((time.time() - start) / 60, 2)} min \n")
