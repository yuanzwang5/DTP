...
if cross_validate == 'part_1':
    data_input = dict(bond_return_ins = torch.from_numpy(df_xret_all[odd_months,:]).float(),
                      ...
                      bd_spread_ins = torch.from_numpy(df_bdspread_all[odd_months,:]).float(),
                      bd_spread_oos = torch.from_numpy(df_bdspread_all[even_months,:]).float(),
    ...
    data_input = dict(bond_return_ins = torch.from_numpy(df_xret_all[even_months,:]).float(),
                      ...
                      bd_spread_ins = torch.from_numpy(df_bdspread_all[even_months,:]).float(),
                      bd_spread_oos = torch.from_numpy(df_bdspread_all[odd_months,:]).float(),
                      num_firm = df_char_all.shape[1],
                      num_feature = num_char)
        ...
        for a1_a2 in list_a1_a2:
            ...
            for n_layer in range(i, i + 1):
                ...
                for n_deep_factors in range(1, 2):
                    ...
                    list_loss_train,list_loss_valid,w_df_train,_,_,_,_,F_train,F_valid,best_loss = run_train_valid_with_batch(data_input, tensor_input_train, tensor_input_valid, learning_rate, epochs=epochs, batch_size=batch_size)
                    ...
                    np.savetxt(
                        '{}/factor_lr{}_dr{}_a1{}_a2{}_gamma{}_L{}_D{}_seed{}.txt'.format(
                            output_dir, learning_rate, dropout_rate, a1, a2, gamma, n_layer, n_deep_factors, seed), F_whole_np)

                    np.savetxt(
                        '{}/list_loss_valid_lr{}_dr{}_a1{}_a2{}_gamma{}_L{}_D{}_seed{}.txt'.format(
                            output_dir, learning_rate, dropout_rate, a1, a2, gamma, n_layer, n_deep_factors, seed), list_loss_valid)

                    np.savetxt(
                        '{}/list_loss_train_lr{}_dr{}_a1{}_a2{}_gamma{}_L{}_D{}_seed{}.txt'.format(
                            output_dir, learning_rate, dropout_rate, a1, a2, gamma, n_layer, n_deep_factors, seed), list_loss_train)
        
                    np.savetxt(
                        '{}/loss_valid_lr{}_dr{}_a1{}_a2{}_gamma{}_L{}_D{}_seed{}.txt'.format(
                            output_dir, learning_rate, dropout_rate, a1, a2, gamma, n_layer, n_deep_factors, seed), [loss_valid])

                    np.savetxt(
                        '{}/loss_best_lr{}_dr{}_a1{}_a2{}_gamma{}_L{}_D{}_seed{}.txt'.format(
                            output_dir, learning_rate, dropout_rate, a1, a2, gamma, n_layer, n_deep_factors, seed), [best_loss])

                    print(f'Save_success for n_layer = {n_layer}, n_deep_factors = {n_deep_factors}, learning_rate = {learning_rate}, dropout_rate = {dropout_rate}, a1 = {a1}, a2 = {a2}, gamma = {gamma}, seed = {seed}')
