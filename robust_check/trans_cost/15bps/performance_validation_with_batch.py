...
for bm_type in benchmark_list:
    for char_type in chars_use_list:
        for port_type in port_type_list:
            ...
            df_bbw_all[:,0] = df_mkt_all.reshape(-1) # replace the market by our market
            ...
            for gamma in list_gamma:
                ...
                ### Determine each layer case best hyperparameters combination
                if gamma == 0.0:
                    list_dropout = [0.1]
                    list_a1_a2 = [(50, 5)]
                    list_learning = [0.001, 0.005, 0.01] 
                    batches = 30
                elif gamma == 0.1:
                    list_dropout = [0.1]
                    list_a1_a2 = [(50, 5)]
                ...
                save_dir = './results_{}/{}_with_batch_{}/{}'.format(char_type, port_type, batches, bm_type)
                ...
                for n_layer in range(1, max_layers+1):
                            ...
                            for dropout_rate in list_dropout:
                                for a1_a2 in list_a1_a2:
                                    ...
                                    for cross_validate in cross_validation_list:
                                        ...
                                        file_name = '{}/factor_lr{}_dr{}_a1{}_a2{}_gamma{}_L{}_D{}_seed{}.txt'.format(
                                                output_dir, learning_rate, dropout_rate, a1, a2, gamma, n_layer, n_deep_factors, seed)

                                        loss_name = '{}/loss_valid_lr{}_dr{}_a1{}_a2{}_gamma{}_L{}_D{}_seed{}.txt'.format(
                                                output_dir, learning_rate, dropout_rate, a1, a2, gamma, n_layer, n_deep_factors, seed)

                                        best_loss = '{}/loss_best_lr{}_dr{}_a1{}_a2{}_gamma{}_L{}_D{}_seed{}.txt'.format(
                                                output_dir, learning_rate, dropout_rate, a1, a2, gamma, n_layer, n_deep_factors, seed)
                    ...
                    df_loss.to_csv(f'{save_dir}/validation_loss_summary_{n_layer}_layer_gamma_{gamma}_seed{seed}.csv', index=False, encoding='utf_8_sig')
                ...
                best_frame = []
                for n_layer in range(1, max_layers+1):
                    df_loss = pd.read_csv(f'{save_dir}/validation_loss_summary_{n_layer}_layer_gamma_{gamma}_seed{seed}.csv')
                ...
                df_best.to_csv(f'{save_dir}/best_summary_gamma_{gamma}_seed{seed}.csv', index=False, encoding='utf_8_sig')
                print(df_best)
