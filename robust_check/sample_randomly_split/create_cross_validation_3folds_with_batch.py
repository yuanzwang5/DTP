...
for filename in ['main_validation_3folds_with_batch.py']:
                ...
                for port_type in port_type_list:
                    for oos_target in oos_target_list:
                        for cross_validate in cross_validation_list:
                            ...
                            line_count = 1
                            f0 = open(filename, 'r')
                            f = open('script_validation_3folds_with_batch_' + str(counter) + '.py', 'w')
                            ...
                            f.write('list_learning = {}'.format(list_learning)+'\n')
...
f = open('submit_validation_3folds_with_batch.sh', 'w')
f.write("#!/bin/bash \n")
f.write('start_time=$(date +"%Y-%m-%d %H:%M:%S") \n')

pre_ = f'seq 1 {counter} | parallel -j{counter} '
f.write(pre_+'python3 script_validation_3folds_with_batch_{}.py > log_valid_3folds_with_batch.txt 2>&1\n')

for i in range(2, counter+1):
    f.write("rm script_validation_3folds_with_batch_{}.py \n".format(i))
...
f.write('echo "############################  This is 3folds Validation Test #################################" >> log_time.txt \n')
