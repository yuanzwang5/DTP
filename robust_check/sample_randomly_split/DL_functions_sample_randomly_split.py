...
class Net_SR(torch.nn.Module):  
    ...
    def forward(self, x, 
        ...
        x = self.output(x) # output deep characteristics
        x = self.batchnorm(x)
...
def run_train_valid_with_batch(data_input,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_loader:
            ...
            # Pass batch_bond_ret_ins to the model
...
def run_ins_oos_with_batch(data_input,
                           tensor_input_ins, tensor_input_oos,
                           learning_rate, 
                           state=False, patience=30, epochs=300, min_delta=0.01, batch_size=30):
...
def winsorize_by_row(df, lower_quantile=0.025, upper_quantile=0.975):
    if isinstance(df, np.ndarray):
        df_ = pd.DataFrame(df)
    else:
        df_ = df.copy()
    winsorized_df = df_.copy()
    
    for index, row in df_.iterrows():
        q_low = row.quantile(lower_quantile)
        q_high = row.quantile(upper_quantile)
        
        row_clipped = row.clip(lower=q_low, upper=q_high)
        
        winsorized_df.loc[index] = row_clipped
        
    if isinstance(df, np.ndarray):
        return winsorized_df.values
    else:
