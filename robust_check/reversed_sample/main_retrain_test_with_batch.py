...
for port_type in port_type_list:
    ...
    ins_return = np.nan_to_num(winsorize_by_row(df_xret_all[T_OOS:,:]) - df_rf_all_use[T_OOS:,:], nan=0.0)
    oos_return = np.nan_to_num(df_xret_all[:T_OOS,:] - df_rf_all_use[:T_OOS,:], nan=0.0)
    ...
    tensor_input_ins = torch.from_numpy(df_char_all[T_OOS:,:,:]).float()
    tensor_input_oos = torch.from_numpy(df_char_all[:T_OOS,:,:]).float()

    df_benchmark_ins = dict_bm[bm_type][T_OOS:,:]
    df_benchmark_oos = dict_bm[bm_type][:T_OOS:,:]
