...
T_ALL = T_INS + T_OOS
odd_months = np.arange(T_OOS, T_OOS+T_VALID, 1)
even_months = np.arange(T_OOS+T_VALID, T_ALL, 1)
...
df_return_all = np.array(df_char_all['monthly_return']).reshape((T_ALL, -1))
...
if cross_validate == 'part_1':
    ...
    ins_return = np.nan_to_num(winsorize_by_row(df_xret_all[odd_months,:]) - df_rf_all_use[odd_months,:], nan=0.0)
    oos_return = np.nan_to_num(winsorize_by_row(df_xret_all[even_months,:]) - df_rf_all_use[even_months,:], nan=0.0)
    ...
    ins_return = np.nan_to_num(winsorize_by_row(df_xret_all[even_months,:]) - df_rf_all_use[even_months,:], nan=0.0)
    oos_return = np.nan_to_num(winsorize_by_row(df_xret_all[odd_months,:]) - df_rf_all_use[odd_months,:], nan=0.0)
